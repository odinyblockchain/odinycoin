...
*Setup instructions for a gitian build of Odinycoin Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Odinycoin
...
to the Odinycoin GitHub release page.
...
- [Building Odinycoin Core](#building-odinycoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/odinycoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/odinycoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/odinycoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/odinycoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/odinycoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/odinycoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/odinycoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@odinycoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/odinycoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/odinycoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/odinycoin-osx-*-build.assert
```
