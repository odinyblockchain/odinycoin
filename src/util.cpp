...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "odinycoin" is a composite category enabling all Odinycoin-related debug output
            if (ptrCategory->count(std::string("odinycoin"))) {
...
#else
    const char* pszModule = "odinycoin";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Odinycoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Odinycoin
// Mac: ~/Library/Application Support/Odinycoin
// Unix: ~/.odinycoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Odinycoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "odinycoin.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty odinycoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override odinycoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "odinycoind.pid"));
