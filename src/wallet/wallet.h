...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ODC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zODC send
enum ZerocoinSpendStatus {
    ZODC_SPEND_OKAY = 0,                            // No error
    ZODC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZODC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZODC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZODC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZODC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZODC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZODC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZODC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZODC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZODC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZODC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZODC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZODC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZODC_SPENT_USED_ZODC = 14,                      // Coin has already been spend
    ZODC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZODC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZODCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzODCTracker> zpivTracker;
    ...
    void setZWallet(CzODCWallet* zwallet);
    CzODCWallet* getZWallet();
    ...
    /// Get 10000 ODC output and keys which can be used for the Masternode
    ...
    /** zODC reset */
    boost::signals2::signal<void()> NotifyzODCReset;
