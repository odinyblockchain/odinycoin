...
public:
    ...
    bool WriteZODCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZODCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZODCSeed_deprecated(uint256& seed);
    bool EraseZODCSeed();
    bool EraseZODCSeed_deprecated();

    bool WriteZODCCount(const uint32_t& nCount);
    bool ReadZODCCount(uint32_t& nCount);
