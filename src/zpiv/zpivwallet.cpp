...
CzODCWallet::CzODCWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZODCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZODCSeed_deprecated()) {
                    LogPrintf("%s: Updated zODC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzODCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZODCCount(nCountLastUsed);
    else if (!walletdb.ReadZODCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzODCWallet::Lock()
{
    seedMaster = 0;
}

void CzODCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzODCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZODC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzODCWallet::LoadMintPoolFromDB()
{
...
void CzODCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzODCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzODCWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzODCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZODC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzODCWallet::SeedToZODC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzODCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzODCWallet::GenerateDeterministicZODC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzODCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzODCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzODCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
