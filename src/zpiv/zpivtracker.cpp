// Copyright (c) 2018-2020 The Odinycoin developers
...
CzODCTracker::CzODCTracker(std::string strWalletFile)
{
...
CzODCTracker::~CzODCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzODCTracker::Init()
{
...
bool CzODCTracker::Archive(CMintMeta& meta)
{
...
bool CzODCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzODCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzODCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzODCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzODCTracker::GetSerialHashes()
{
...
CAmount CzODCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzODCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzODCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzODCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzODCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzODCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzODCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzODCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzODCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzODCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzODCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzODCWallet* zODCWallet)
{
    bool iszODCWalletInitialized = (NULL != zODCWallet);
    CMintMeta meta;
    ...
    if (! iszODCWalletInitialized)
        zODCWallet = new CzODCWallet(strWalletFile);
    meta.isSeedCorrect = zODCWallet->CheckSeed(dMint);
    if (! iszODCWalletInitialized)
        delete zODCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzODCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzODCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzODCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzODCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzODCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzODCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzODCWallet* zODCWallet = new CzODCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zODCWallet);
        }
        delete zODCWallet;
