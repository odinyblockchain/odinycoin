...
#include "qt/odinycoin/privacywidget.h"
#include "qt/odinycoin/forms/ui_privacywidget.h"
#include "qt/odinycoin/qtutils.h"
#include "guiutil.h"
#include "qt/odinycoin/denomgenerationdialog.h"
#include "qt/odinycoin/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(OdinycoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zODC anonymizes your ODC by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zODC or convert back to ODC"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 ODC ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zODC");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zODC");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zODC");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zODC");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zODC");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zODC");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zODC");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zODC");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zODC Movements"));
    ...
    // Only Convert to ODC enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzODC->setTitleClassAndText("btn-title-grey", "Total 0 zODC");
    ui->btnTotalzODC->setSubTitleClassAndText("text-subtitle", "Show denominations of zODC owned.");
    ui->btnTotalzODC->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select ODC outputs to mint into zODC.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zODC");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzODC, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zODC");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of ODC to mint into zODC"));
    }else{
        btnText = tr("Convert back to ODC");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zODC to convert back into ODC"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzODC->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzODC->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zODC is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zODC, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zODC").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zODC minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zODC converted back to ODC"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any ODC to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zODC </b>";
    ...
    ui->btnTotalzODC->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
