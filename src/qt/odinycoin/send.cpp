...
#include "qt/odinycoin/send.h"
#include "qt/odinycoin/forms/ui_send.h"
#include "qt/odinycoin/addnewcontactdialog.h"
#include "qt/odinycoin/qtutils.h"
#include "qt/odinycoin/sendchangeaddressdialog.h"
#include "qt/odinycoin/optionbutton.h"
#include "qt/odinycoin/sendconfirmdialog.h"
#include "qt/odinycoin/myaddressrow.h"
#include "qt/odinycoin/guitransactionsutils.h"
...
SendWidget::SendWidget(OdinycoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (ODC)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("Odinycoin address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 ODC");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_ODC : AskPassphraseDialog::Context::Send_zODC, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zODC is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zODC selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zODC transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZODC_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zODC require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zODC transaction
        ...
        emit message("zODC transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isODC){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zODC in your balance to select."));
        }
    }
}
...
void SendWidget::onODCSelected(bool _isODC){
    isODC = _isODC;
    setCssProperty(coinIcon, _isODC ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
