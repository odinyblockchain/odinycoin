...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::ODC);
    ...
    if (settings.contains("nAnonymizeOdinycoinAmount") || reset)
        SoftSetArg("-anonymizeodinycoinamount", settings.value("nAnonymizeOdinycoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in odinycoin.cpp during shotdown to also remove the window positions
